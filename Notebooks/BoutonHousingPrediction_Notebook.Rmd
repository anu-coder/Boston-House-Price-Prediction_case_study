...
```{r}
plot(Bhousing[,c(1,3,5,6,8,13,14)], pch= 8)
```

We see from the last column, that MEDV has more or less a positive or negetive correlation with all other variables we have plotted. Also another thing that needs to be noticed is that all other variables are also correlated to each other. This gives us an intuition that we can probably apply dimension reducibility techniques i.e. PCA or LDA etc.
...
The summary show a significant difference in the values of the quartiles. Let us now choose the statistics that we would be using to determine the effeicienct of the model. 
...
To do all this we shall use the "caret" library and use a inbuilt method "linear model(lm)" and also use "repeated cross validation", dividing the data set into 10 folds and repeat 10 times.
...
model <- train(MEDV~., data=Bhousing, trControl=train_control, method="lm")
...
model <- train(log(MEDV)~., data=Bhousing, trControl=train_control, method="lm")
...
model <- train(log(MEDV) ~ DIS+RAD+LSTAT+NOX+INDUS+CRIM+TAX, data=Bhousing, trControl=train_control, method="lm")
...
Let us now apply PCA in this and then apply the model.
...
model <- train(log(MEDV) ~ pc1, data=pc.data, trControl=train_control, method="lm")
